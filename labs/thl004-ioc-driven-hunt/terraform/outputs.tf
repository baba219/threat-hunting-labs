output "vm_ip" {
  value       = google_compute_instance.thl_ioc_lab.network_interface[0].access_config[0].nat_ip
  description = "VM external IP"
}

output "kibana_url" {
  value       = "http://${google_compute_instance.thl_ioc_lab.network_interface[0].access_config[0].nat_ip}"
...
output "vm_name" {
  value       = google_compute_instance.thl_ioc_lab.name
