def step_3_check(handles:, resources:, maximum_score:)
  begin
    require "json"
    ...
    target_title = "THL03-Mailbox-Persistence"
    ...
    kid_cmd = <<~BASH
      set -e
      KID=$(sudo docker ps --format '{{.ID}} {{.Names}}' | awk '$2 ~ /(^|[_-])kibana([_-]|$)/ {print $1; exit}')
      if [ -z "$KID" ]; then
    ...
    types_to_try.each do |t|
      cmd = <<~BASH
        set -e
        sudo docker exec -i "#{kid}" curl -s -H "kbn-xsrf: true" "http://localhost:5601/api/saved_objects/_find?type=#{t}&per_page=10000&search_fields=title&search=#{target_title}"
      BASH
    ...
    return { done: false, score: 0, message: "#{e.class} - #{e.message}", student_message: "no_kibana" }
  end
end